pipeline {
  ...
  stages {
    ...
    stage('Codedeploy') {
      steps {
        step([$class: 'AWSCodeDeployPublisher', applicationName: 'project01-production-in-place', deploymentConfig: "${DEPLOYMENT_CONFIG_NAME}", deploymentGroupAppspec: false, deploymentGroupName: 'project01-production-in-place', region: 'ap-northeast-1', waitForCompletion: false])
      }
    }
  }  
}
